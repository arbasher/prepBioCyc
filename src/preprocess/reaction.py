...
class Reaction(object):
    def __init__(self, file_name_reaction='reactions.dat', file_name_ec_reaction='reaction-links.dat'):
        """ Initialization
        :type file_name_reaction: str
        :param file_name_reaction: file name for the reaction
        """
        self.reaction_file_name = file_name_reaction
        self.ec_reaction_file_name = file_name_ec_reaction
        self.reaction_info = OrderedDict()

    def process_reactions(self, reaction_idx_list_ids, list_ids, data_path, verbose=True):
        """ process reactions data
        :type data_path: str
        :param data_path: file name for the reaction
        """
        reaction_file = os.path.join(data_path, self.reaction_file_name)
                    ...
                    if not str(text).startswith('#'):
                        ...
                        if ls:
                            if ls[0] == 'UNIQUE-ID':
                                ...
                                if reaction_id not in list_ids[reaction_idx_list_ids]:
                                    list_ids[reaction_idx_list_ids].update(
                                        {reaction_id: len(list_ids[reaction_idx_list_ids])})
    ...
    def add_ec_info(self, ec_idx_list_ids, list_ids, data_path, verbose=True):
        """ process reactions data
        :type data_path: str
        :param data_path: file name for the reaction
        """
        ec_reaction_file = os.path.join(data_path, self.ec_reaction_file_name)
                ...
                for text in f:
                    if not str(text).startswith('#'):
                        ...
                        if ls:
                            ...
                            for ec in ls[1:]:
                                ec = ''.join(ec.split('|'))
                                if ec not in list_ids[ec_idx_list_ids]:
                                    list_ids[ec_idx_list_ids].update({ec: len(list_ids[ec_idx_list_ids])})
    ...
    def add_gene_info(self, genes_info, gene_name_id_position_idx, reaction_position_idx):
        print('\t\t\t--> Adding genes names id to reactions')
        tmp_enzrxns_genes = OrderedDict()
        for (g_id, g_item) in genes_info.items():
            lst_reaction = g_item[reaction_position_idx][1]
            if lst_reaction:
                for r in lst_reaction:
                    if r not in tmp_enzrxns_genes:
                        tmp_enzrxns_genes.update({r: [g_item[gene_name_id_position_idx][1]]})
                    elif r in tmp_enzrxns_genes:
                        if g_item[gene_name_id_position_idx][1] not in tmp_enzrxns_genes[r]:
                            tmp_enzrxns_genes[r].extend([g_item[gene_name_id_position_idx][1]])
